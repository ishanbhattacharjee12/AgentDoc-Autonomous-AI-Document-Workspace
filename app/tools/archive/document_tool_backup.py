...
def _generate_pdf(filepath: Path, title: str, document_type: str, assumptions: list[str], content: str, goal: str, filename: str) -> str:
    ...
    # Content - structured Markdown to PDF with FPDF table rendering
    if DEBUG_TIMING:
        ...
        if line.startswith('## '):
            ...
            if table_rows:
                try:
                    ...
                    heading_face = FontFace(fill_color=(230, 235, 245), color=(44, 62, 80), emphasis="B")
                    # Clean markdown and replace text encoding
                    cleaned_rows = []
                    for row in table_rows:
                        cleaned_row = []
                        for cell in row:
                            cell_cleaned = _clean_markdown(cell).replace('**', '').encode('latin-1', 'ignore').decode('latin-1')
                    ...
                    with pdf.table(
                        ...
                        pass
                    pdf.ln(6) # Breathing room after table
