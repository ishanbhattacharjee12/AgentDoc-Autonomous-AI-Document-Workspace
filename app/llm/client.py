...
def call_llm_json(
    ...
    try:
        ...
        logger.warning("JSON parse failed on first attempt: %s", e)
    ...
    raw_retry = call_llm(system_prompt, repair_prompt, temperature=0.2, max_tokens=max_tokens)
    cleaned_retry = clean_json_response(raw_retry)

    try:
        return json.loads(cleaned_retry)
    except json.JSONDecodeError as e:
        logger.error("JSON parse failed on repair attempt: %s", e)
...
- Day 90: Final review — decide on continued investment vs. pivot"""
    ...
    if "best practice" in prompt_lower or "knowledge" in system_prompt.lower() or "industry" in prompt_lower or "research" in task_name.lower() if 'task_name' in dir() else False:
        if "chatbot" in prompt_lower:
